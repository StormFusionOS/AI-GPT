...
def login(data: LoginRequest) -> TokenPair:
    ...
    with session_scope():
        ...
        access_token = create_token({"sub": str(user.id), "role": user.role.value}, settings.access_token_ttl())
        refresh_token = create_token({"sub": str(user.id), "role": user.role.value, "scope": "refresh"}, settings.refresh_token_ttl())
